...
- The planned admin, newsroom, internationalization, and Your Access features.
...
- Rebuild the legacy Your Access map as a polished, accessible feature backed
  by database content.
...
- `v2` is the integration branch for the new website.
- Short-lived feature branches should merge into `v2`.
...
- `old-funding`: legacy Funding implementation and Your Access map reference.
...
- Countries and regions used by Your Access.
- Services available in each country.
- Country summaries and detailed country-page content.
- Media uploaded for those entities.

The following may also move into Supabase if colleagues need to maintain them:
...
That scope should be confirmed against the Figma page inventory before the
schema is finalized.
...
The exact launch locales remain to be confirmed.
...
Your Access is a discoverable view of the company's geographic presence and
services.
...
- End-to-end tests for:
  ...
  - Your Access selection and country navigation.
...
- Obtain Figma access.
- Inventory pages, responsive states, and interactions.
...
- Confirm Your Access country/service content requirements.
...
- Build an early Your Access map interaction prototype to retire technical
  risk.
...
- Rebuild Your Access instead of copying the legacy component wholesale.
- Use stable ISO country identifiers.
- Implement the new Figma design as the visual source of truth.

## 22. Recommended defaults awaiting confirmation

- English is the unprefixed default locale.
- German and Italian are the other initial locales.
- Invite-only email authentication for colleagues.
- `admin` and `editor` roles initially.
- One canonical `people` system shared by team profiles and article authors.
- Per-translation publication state for localized editorial content.
- `d3-geo` and local topology for the Your Access map.
...
1. Exact launch locales and whether all migrated content must be translated at
   launch.
2. Whether editors can publish directly or require admin approval.
3. Email/password versus passwordless email login.
4. Final CMS scope beyond team, articles, countries, and services.
5. Final public naming and routes: for example, Newsroom versus Articles or
   Insights.
6. Whether static route segments should be translated or only locale-prefixed.
7. Exact Your Access country fields, service descriptions, contacts, and calls
   to action.
8. Whether country selection opens a modal, side panel, or another interaction
   pattern in the final design.
9. Search requirements for the newsroom.
10. Analytics, consent, monitoring, SMTP, and spam-protection providers.
11. Whether legal pages are code-managed or admin-managed.
12. Post-launch Git and persistent Supabase development branch naming.
...
- Your Access is complete, accessible, responsive, and database-backed.
...
1. Resume implementation from `v2`, not the legacy `main` application.
2. Confirm the open locale, permissions, CMS scope, and URL decisions.
3. Obtain Figma access and produce the page/component/content inventory.
4. Stabilize and pin the `v2` foundation.
5. Write the detailed Supabase schema and RLS plan.
6. Add the first hosted Supabase migrations.
7. Implement the admin/auth shell.
8. Deliver the team-management vertical slice.
